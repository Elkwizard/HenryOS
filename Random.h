...
class Random {
    public:
        ...
        static float perlin2D(float x, float y, float f = 1, float seed = Random::seed) {
            x *= f;
            y *= f;
            return Interpolation::quadLerp(s_p2D(x, y, seed), s_p2D(x + 1, y, seed), s_p2D(x, y + 1, seed), s_p2D(x + 1, y + 1, seed), noiseTCorrect(fmod(x, 1.0f)), noiseTCorrect(fmod(y, 1.0f)));
        ...
        static float perlin3D(float x, float y, float z, float f = 1, float seed = Random::seed) {
            ...
            return Interpolation::cubeLerp(
                ...
                fmod(x, 1.0f), fmod(y, 1.0f), fmod(z, 1.0f));
        }
        struct VoronoiCell {
            float x, y, z;
        };
        ...
        static VoronoiCell getVoronoiCell2D(float x, float y) {
            return { 
                floor(x) + seedRand(floor(x) + floor(y) * 1000), 
                floor(y) + seedRand(floor(y) + floor(x) * 10000) 
            };
        }
...
float Random::seed = 238958.489f * (rand() % 10000) / 1000.0f;
